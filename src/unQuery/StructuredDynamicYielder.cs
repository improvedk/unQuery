		...
		public IEnumerator<SqlDataRecord> GetEnumerator()
		{
			...
			foreach (object row in values)
			{
				...
				if (record == null)
				{
					...
					if (typeof(ITypeHandler).IsAssignableFrom(valueType))
					{
						unQueryType = true;
						recordValues = new object[1];
						record = new SqlDataRecord(((ITypeHandler)row).CreateMetaData(""));
					}
					else if (unQueryDB.ClrTypeHandlers.ContainsKey(valueType))
					{
						simpleType = true;
						recordValues = new object[1];
						record = new SqlDataRecord(unQueryDB.ClrTypeHandlers[valueType].CreateMetaData(""));
					}
					else
					{
				...
				if (unQueryType)
				{
					if (row == null)
						...
						record.SetValue(0, ((ISqlType)row).GetRawValue());
				}
				else if (simpleType)
				{
